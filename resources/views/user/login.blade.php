    ...
    <div class="bg-black/60 w-full h-screen flex justify-center items-center">
      <div class="flex">
        ...
        <div class="flex flex-col items-center justify-center bg-white p-6">
          <form action="{{ route('user.login.submit') }}" method="POST" class="flex flex-col gap-4">
            @csrf
            <input name="email" placeholder="email" type="text" class="w-full border-1 rounded-md p-2">
            <input name="password" placeholder="password" type="password" class="w-full border-1 rounded-md p-2">
            <div>
              <x-form.submit-btn>Sign In</x-form.submit-btn>
              <p>
                Don't have an account?
                <a data-redirect="{{ route('user.home') }}" href="{{ route('user.register') }}" class="underline">Sign Up</a>
              </p>
            </div>
          </form>
        </div>
